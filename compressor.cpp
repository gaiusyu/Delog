...
public:
    ...
    PatternRecognizer(const std::string& logname) {
        ...
        regex_map["Android"] = { { R"((\d+)-(\d+) (\d+):(\d+):(\d+).(\d+))"}, {"<T>"} };
        regex_map["Apache"] = { {R"((\d+)\.(\d+)\.(\d+)\.(\d+))", R"((\d{2}) (\d+):(\d+):(\d+))"}, {"<I>", "<T>"} };
        regex_map["BGL"] = { {R"((\d+)-(\d+)-(\d+)-(\d+)\.(\d+)\.(\d+))", R"((\d{4})\.(\d+)\.(\d+))"}, { "<A>", "<F>"} };
        regex_map["Hadoop"] = { {R"((\d+)\-(\d+)\-(\d+))", R"((\d+):(\d+):(\d+),(\d+))"}, {"<D>", "<T>"} };
        regex_map["HDFS"] = { { R"((\d+)\.(\d+)\.(\d+)\.(\d+))", R"((\d+):(\d+):(\d+),(\d+))", R"(blk_-?\d+)"}, {"<I>", "<T>", "<B>"} };
        regex_map["HealthApp"] = { { R"((\d+):(\d+):(\d+):(\d+))"}, {"<T>"} };
        regex_map["HPC"] = { {R"(\d{10})"}, {"<T>"} };
        regex_map["Linux"] = { {R"((\d+)\.(\d+)\.(\d+)\.(\d+))", R"((\d+):(\d+):(\d+))", R"(rhost=([^\s]+))"}, {"<I>", "<T>", "<X>"} };
        regex_map["Mac"] = { {R"((\d+)-(\d+)-(\d+)-(\d+))", R"((\d+):(\d+):(\d+)(?:\.(\d+))?)"}, {"<D>", "<T>"} };
        regex_map["OpenSSH"] = { {R"((\d+)\.(\d+)\.(\d+)\.(\d+)(?![.\d]))", R"((\d+) (\d{2}):(\d{2}):(\d{2}))"}, {"<I>", "<T>"} };
        regex_map["OpenStack"] = { { R"(\.(\d+)-(\d+)-(\d+)_(\d+):(\d+):(\d+))",R"((\d+)-(\d+)-(\d+).(\d+):(\d+):(\d+)\.(\d+))",R"((\d+)\.(\d+)\.(\d+)\.(\d+))"}, { "<D>", "<T>", "<I>"} };
        ...
        regex_map["Thunderbird"] = { { R"((\d+):(\d+):(\d+))",R"((\d{4}\.)(\d+)\.(\d+))"}, {"<I>","<B>","<A>"}};
        regex_map["Windows"] = { { R"((\d+)\.(\d+)\.(\d+)\.(\d+))",R"((\d+)-(\d+)-(\d+) (\d+):(\d+):(\d+))", R"((\d+):(\d+):(\d+))"}, {"<E>","<T>","<D>"} };
        regex_map["Zookeeper"] = { { R"((\d+)\.(\d+)\.(\d+)\.(\d+))",R"((\d+)-(\d+)-(\d+) (\d+):(\d+):(\d+),(\d+))"}, {"<I>","<T>"} };

...
def main():
    ...
    # --- 路径处理 ---
    logshrink_run_dir = os.path.join("LogShrink", "python_compression")
    
    if not os.path.isdir(logshrink_run_dir):
        print(f"Error: Directory '{logshrink_run_dir}' not found. Please ensure this script is in the same directory as the 'LogShrink' folder.", file=sys.stderr)
        sys.exit(1)

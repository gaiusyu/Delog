...
class RuleFinder:
    ...
    def gen_basic_info(self):
        ...
        for column in self.df.columns:
            # 假设整列都是数字，如果发现任何一个非数字值，则判定为字符串列
            ...
            for value in self.df[column]:
                try:
                    ...
                    # 检查是否为整数（而不是浮点数）并且在32位有符号整数范围内
                    if val_int != val_float or abs(val_int) >= (1 << 31):
                        is_numeric_column = False
                        break # 发现不符合条件的值，此列不是数字列，跳出内部循环
                
                except (ValueError, TypeError):
                    # 如果转换失败（例如遇到'₃', 'abc'或None），则此列不是数字列
                    is_numeric_column = False
                    break # 跳出内部循环
